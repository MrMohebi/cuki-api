...
if(isset($_POST['token'])){
    ...
    $sqlUpdateResInfoParams = array(
        ...
        "phone"=> (is_array($phone)) ? json_encode($phone) : json_encode(array($phone)),
        ...
        "employers"=> count($employers) > 0 ? $employers : $previousResInfo["employers"],
        "social_links"=> count($socialLinks) > 0  ? $socialLinks : $previousResInfo["social_links"],
        "open_time"=> count($openTime) == 7  ? $openTime : $previousResInfo["open_time"],
        "type"=> count($type) > 0  ? $type : $previousResInfo["type"],
